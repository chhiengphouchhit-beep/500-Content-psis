...
print(f"Total titles to format cleanly: {len(titles)}")
...
def get_category(t):
    ...
    if any(w in t for w in ['Campus', 'POV', '៧ ព្រឹក', '៧:۰۰', 'Hangout', 'សាខា', 'TK', 'TTP', 'CAP', 'RSK', 'NR3', 'Behind the Scenes', 'International Day', 'ចូលឆ្នាំ', 'Halloween', 'Christmas', 'ភ្ជុំបិណ្ឌ', 'Nurse', 'Canteen', 'Field Trip', 'រថយន្តក្រុង', 'Bus', 'Playground', 'អាហារថ្ងៃត្រង់']):
...
# Dump with proper escaping
cards_js_str = json.dumps(cards_list, ensure_ascii=False, indent=2)
...
# Bump localStorage version to v20
html = html.replace('psis_topics_data_v19', 'psis_topics_data_v20')
html = html.replace('psis_comments_data_v19', 'psis_comments_data_v20')
html = html.replace('psis_topics_data_v18', 'psis_topics_data_v20')
html = html.replace('psis_comments_data_v18', 'psis_comments_data_v20')
...
if res.returncode == 0:
    print("SUCCESS: Node syntax check passed cleanly! No JavaScript syntax errors!")
else:
