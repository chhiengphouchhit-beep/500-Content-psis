...
print(f"Compact formatting for {len(titles)} titles...")
...
for i, t in enumerate(titles):
    ...
    card_obj = {
        ...
        "script": {
            "hook": f"\"{t}\"",
            "context": "នៅ PSIS កម្មវិធីអប់រំត្រូវបា​នរៀបចំឡើងយ៉ាងផ្Citeផ្ចង់ ដោយបញ្ចូលវិធីសាស្ត្របង្រៀន និងសកម្មភាពជាក់ស្តែង...",
...
# Compact JSON without huge indentations
cards_js_str = json.dumps(cards_list, ensure_ascii=False)
...
# Bump localStorage version to v22
html = html.replace('psis_topics_data_v21', 'psis_topics_data_v22')
html = html.replace('psis_comments_data_v21', 'psis_comments_data_v22')
html = html.replace('psis_topics_data_v20', 'psis_topics_data_v22')
html = html.replace('psis_comments_data_v20', 'psis_comments_data_v22')

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)

# Also create zip file psis_content_planner.zip
with zipfile.ZipFile('psis_content_planner.zip', 'w', zipfile.ZIP_DEFLATED) as zipf:
    zipf.write('index.html')

print("Compact index.html & psis_content_planner.zip created successfully!")
